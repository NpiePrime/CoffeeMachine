...
public class Main {
    ...
    public static void addInventory() {
	    //Read in amt coffee
	    int coffeeQty = parseQuantity(inputOutput("\nPlease enter the units of coffee to add: "));
	    	    
	    //Read in amt milk
	    int milkQty = parseQuantity(inputOutput("\nPlease enter the units of milk to add: "));
	    	    
	    //Read in amt sugar
	    int sugarQty = parseQuantity(inputOutput("\nPlease enter the units of sugar to add: "));
	    	    
	    //Read in amt chocolate
	    int chocolateQty = parseQuantity(inputOutput("\nPlease enter the units of chocolate to add: "));
    ...
    private static int parseQuantity(String qtyString) {
    	int amt = 0;
    	try {
    		amt = Integer.parseInt(qtyString);
    	} catch (NumberFormatException e) {
    		return 0;
    	}
		if (amt >= 0) {
